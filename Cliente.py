...
class Cliente(rpyc.Service):
    def __init__(self, ip_cliente, porta_cliente, ip_servidor, porta_servidor):
        ...
        # Varíavel booleana que guarda o estado quanto ao modo passivo
        self.isModoPassivo = False # True, caso esteja no modo passivo. Falso, caso contrário
        ...
        self.clientes_renderizados = (
            {}
        )  # Key: clientname. Value: buffer vídeo desse cliente

        # Dicionário que armazena as threads de videos renderizados (recepção de vídeo) de cada cliente
        self.threads_videosRenderizados = (
            {}
        )  # Key: Clientname que transmite. Value: Thread designada ao mesmo que recebe
    ...
    # // Saída: Retorna true caso seja possível fazer a conexão ou Falso, caso contrário
    def _init_conexaoServidor(self):
        try:
    ...
    def setNickname(self, nickname):
        ...
        return self.conn.root.registrarConexao(
            nickname, self.ip_cliente, self.porta_cliente
        )
    ...
    # Permite o cliente ver a última mensagem recebida do chat
    # // Entrada: Nome do chat
    # // Saída: última mensagem recebida
    def verMensagemRecebida(self, chatname):
        return self.conn.root.msgRecebidaChat(chatname)
    ...
    def transmitirVideo(self, chatname):
        ...
        interromperCompartilhamento_async = rpyc.async_(
    ...
    def _init_modoPassivo(self):
        try:
            self.servidor_cliente = rpyc.ThreadedServer(
                Cliente(
                    self.ip_cliente,
                    self.porta_cliente,
                    self.ip_servidor,
            ...
            print("Não foi possível ligar o Servidor do Cliente (modo passivo) nesse IP: " + 
                self.ip_cliente + "; porta: " + self.porta_cliente)
            return False

        if not self.isModoPassivo:
            self.isModoPassivo = True
            print("-- < ServerClient - Cliente no modo passivo > --")
            self.servidor_cliente.start()   # chamada bloqueante
            self.isModoPassivo = False      # Quando acabar o bloqueio, saiu do modo passivo       
        ...
        if escolha == 1:
            ...
            if cliente.entrarChat(chatname, senha_digitada):
                ...
                while True:
                    ...
                    if msg == "/m":
                        print("<membros>")
                        for membro in cliente.verMembrosChat(chatname):
                            print(membro)
                        print("---------")
                    elif msg == "/v":
...
if __name__ == "__main__":
    IP_CLIENTE = "127.0.0.1"  # Set IP_CLIENTE
    PORTA_CLIENTE = input("Digite sua porta: ")  # Set PORTA_CLIENTE
    IP_SERVIDOR = "127.0.0.1"  # Set IP_SERVIDOR
