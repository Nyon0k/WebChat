...
class Servidor(rpyc.Service):
    def __init__(self, ip_servidor, porta_servidor):
        ...
        self.connections = (
            {}
        )  # Key: clientname. Value: lista com [ip_cliente, porta_cliente]
    ...
    def _init_server(self):
        servidor = rpyc.ThreadedServer(
            Servidor(self.ip_servidor, self.porta_servidor), port=self.porta_servidor
        )
    ...
    def on_connect(self, conn):
        new_cliente = conn._channel.stream.sock.getpeername()
        print(str(new_cliente) + " se conectou ")
        pass
    ...
    def exposed_registrarConexao(self, clientname, ip_cliente, porta_cliente):
        ...
        if cliente_registrado:
            return (
                False  # Não registra a conexão, pois já existe um clientname registrado
            )
        else:
            self.connections[clientname] = [ip_cliente, porta_cliente]
            return (
                True  # registra a conexão, pois não existe nenhum clientname registrado
            )
    ...
    # Retorna a última mensagem recebida do chat #
    # // Entrada: Nome do chat
    # // Saída: última mensagem recebida do chat
    def exposed_msgRecebidaChat(self, chatname):
        return self.chats.get(chatname).getLastMessage()
    ...
    def exposed_compartilharMsg(self, msg, chatname, clientname):
        ...
        for membro in membros_chat:
            if membro != clientname:
                ip_membro, porta_membro = (
                    membros_chat.get(membro)[0],
                    membros_chat.get(membro)[1],
                )
                # Armazena a ultima msg recebida no chat
                chat.msgRecebida(msg, clientname)
    ...
    def exposed_interromperCompartilhamentoVideo(self, clientname, chatname):
        print(
            "< Compartilhamento de vídeo do cliente: "
            + clientname
            + " no chat: "
            + chatname
            + " interrompido >"
